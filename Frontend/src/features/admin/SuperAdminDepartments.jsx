...
const SuperAdminDepartments = ({ addToast }) => {
  ...
  const fetchAnnouncements = async (deptName) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5024/api'}/DepartmentNotifications/${encodeURIComponent(deptName)}`, {
  ...
  const handleDeleteAnnouncement = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5024/api'}/DepartmentNotifications/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {
  ...
  const submitEditAnnouncement = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5024/api'}/DepartmentNotifications/${editingAnnouncement.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleSendNotification = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5024/api'}/DepartmentNotifications/${encodeURIComponent(selectedDepartment.name)}/notify`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
